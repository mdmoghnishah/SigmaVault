...
const InvestmentProducts = () => {
  return (
    <>
      ...
      <section className="max-w-[1100px] mx-auto p-4 sm:p-6 lg:p-8">
      {/* Portfolio Cards */}
      <div className="flex flex-col gap-10">
        {cardData.map((card) => (
          <div
            key={card.title}
            className="flex flex-col md:flex-row items-start rounded-[18px] bg-white px-2 py-4 shadow-sm"
            style={{
              width: "956px",
              opacity: 1,
              border: `3px solid ${card.borderColor}55`,
              gap: "1px",
              minHeight: "289px",
            }}
          >
            <div className="mr-6 mt-2">{card.icon}</div>

            <div className="flex-1">
              <div className="flex flex-col md:flex-row md:justify-between items-start md:items-center mb-2 gap-2">
                <span className="font-sans font-bold text-[30px] leading-[100%] text-black">
                  {card.title}
                </span>
                <div
                  className="grid grid-cols-2 gap-x-2 text-[13px] font-sans"
                  style={{
                    width: "337px",
                    height: "69px",
                    opacity: 1,
                ...
                >
                  ...
                  <div className="text-right pr-30 text-[#06C752] font-semibold">{card.minInvestment}</div>

                  <div>Expected Return:</div>
                  <div className="text-right pr-30 text-[#C70606] font-semibold">
                    {card.expectedReturn}
                  </div>

                  <div>Risk Level:</div>
                  <div className={`${card.risk.color} text-right pr-30 whitespace-nowrap font-semibold`}>
                    {card.risk.text}
                  </div>
                </div>
              </div>

              <p className="font-sans font-normal text-[16px] leading-[100%] tracking-[0] text-[#222] mb-10">
                {card.desc}
              </p>
              <p className="font-sans font-normal text-[16px] leading-[100%] tracking-[0] text-[#222] ">
                {card.desc2}
              </p>
            </div>
          </div>
